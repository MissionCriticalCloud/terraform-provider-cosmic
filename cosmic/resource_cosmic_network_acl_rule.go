...
func resourceCosmicNetworkACLRule() *schema.Resource {
	return &schema.Resource{
		Create: resourceCosmicNetworkACLRuleCreate,
...
func resourceCosmicNetworkACLRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if nrs := d.Get("rule").(*schema.Set); nrs.Len() > 0 {
		...
		rules := resourceCosmicNetworkACLRule().Schema["rule"].ZeroValue().(*schema.Set)
	...
	return resourceCosmicNetworkACLRuleRead(d, meta)
}
...
func resourceCosmicNetworkACLRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	ruleMap := make(map[string]*cosmic.NetworkACL, l.Count)
...
func resourceCosmicNetworkACLRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	// Make sure all required parameters are there
	if err := verifyNetworkACLParams(d); err != nil {
		return err
	}
...
func resourceCosmicNetworkACLRuleDelete(d *schema.ResourceData, meta interface{}) error {
